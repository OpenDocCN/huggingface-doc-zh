...
+   `num_layers`（`int`，*可选*，默认为 24）— Transformer 解码器中隐藏层的数量。
...
+   `encoder_attention_mask`（`torch.LongTensor`，形状为`(batch_size, encoder_sequence_length)`，*可选*）— 用于避免在编码器 input_ids 的填充令牌索引上执行交叉注意力的掩码。选择的掩码值为`[0, 1]`：
