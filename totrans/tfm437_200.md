...
要在HuggingFace中使用这些原始检查点，您可以使用`convert_mistral_weights_to_hf.py`脚本将它们转换为HuggingFace格式：
...
由*config.num_hidden_layers*层组成的Transformer解码器。每一层都是一个`MistralDecoderLayer`。
