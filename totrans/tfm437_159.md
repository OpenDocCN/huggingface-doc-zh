...
+   `pooler_output`（形状为`(batch_size, embeddings_size)`的`tf.Tensor`） - DPR编码器输出*pooler_output*对应于问题表示。序列的第一个标记（分类标记）的最后一层隐藏状态，进一步由线性层处理。此输出用于嵌入问题以进行具有上下文嵌入的最近邻查询。
