...
🤗 数据集可以读取由一个或多个CSV文件组成的数据集（在这种情况下，将CSV文件作为列表传递）：
...
即使没有互联网连接，仍然可以加载数据集。只要之前从Hub存储库下载过数据集，它应该已被缓存。这意味着您可以从缓存中重新加载数据集并离线使用。
