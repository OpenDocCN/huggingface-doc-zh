...
+   `chat_template`（`str`，*可选*）— 用于格式化聊天消息列表的Jinja模板字符串。查看[https://huggingface.co/docs/transformers/chat_templating](https://huggingface.co/docs/transformers/chat_templating)获取完整描述。
...
+   如果批次大小为1，则为`self.token_to_word(token_index)`
...
+   `sequence_index`（`int`，*可选*，默认为0）—如果批次中编码了一对序列，则可以用于指定提供的单词索引属于该对中的哪个序列（0或1）。
...
标记范围以[TokenSpan](/docs/transformers/v4.37.2/en/internal/tokenization_utils#transformers.TokenSpan)形式返回：
...
+   如果批处理大小为1，则为`self.word_to_tokens(word_index, sequence_index: int = 0)`

+   如果批处理大小大于或等于1，则为`self.word_to_tokens(batch_index, word_index, sequence_index: int = 0)`
...
+   `batch_index`（`int`，*可选*，默认为0）— 要访问的批处理中的索引。
