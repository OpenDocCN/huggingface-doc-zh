...
`where` 参数必须表示为比较谓词，可以是：

+   由列名、比较运算符和值组成的简单谓词

    +   比较运算符有：`=`, `<>`, `>`, `>=`, `<`, `<=`

+   由两个或多个简单谓词组成的复合谓词（可选地使用括号分组以指示评估顺序），与逻辑运算符结合
...
+   数据集的一部分 `rows` 和特定行中每列包含的内容。

行按行索引排序。

例如，这里是 `ibm.duorc`/`SelfRC` 训练分割的 `no_answer=true` 条件下匹配的 `rows` 的 `features` 和第 150-151 行的切片：
