...
> 原文：[`huggingface.co/docs/transformers/v4.37.2/en/model_doc/qdqbert`](https://huggingface.co/docs/transformers/v4.37.2/en/model_doc/qdqbert)

## 概述

QDQBERT 模型可以在 Hao Wu、Patrick Judd、Xiaojie Zhang、Mikhail Isaev 和 Paulius Micikevicius 的[整数量化用于深度学习推断：原理和实证评估](https://arxiv.org/abs/2004.09602)中找到。
...
+   QDQBERT 需要[Pytorch 量化工具包](https://github.com/NVIDIA/TensorRT/tree/master/tools/pytorch-quantization)的依赖。要安装`pip install pytorch-quantization --extra-index-url https://pypi.ngc.nvidia.com`
...
QDQBERT 模型通过[Pytorch 量化工具包](https://github.com/NVIDIA/TensorRT/tree/master/tools/pytorch-quantization)中的`TensorQuantizer`向 BERT 添加了伪量化操作（QuantizeLinear/DequantizeLinear ops 对）。`TensorQuantizer`是用于量化张量的模块，`QuantDescriptor`定义了张量应该如何量化。有关更多详细信息，请参阅[Pytorch 量化工具包用户指南](https://docs.nvidia.com/deeplearning/tensorrt/pytorch-quantization-toolkit/docs/userguide.html)。
...
导出到 ONNX 的目标是通过[TensorRT](https://developer.nvidia.com/tensorrt)部署推断。伪量化将被分解为一对 QuantizeLinear/DequantizeLinear ONNX 操作。将 TensorQuantizer 的静态成员设置为使用 Pytorch 自己的伪量化函数后，伪量化模型可以导出到 ONNX，按照[torch.onnx](https://pytorch.org/docs/stable/onnx.html)中的说明操作。示例：
...
这是用于存储 QDQBertModel 配置的配置类。它用于根据指定的参数实例化一个 QDQBERT 模型，定义模型架构。使用默认值实例化配置将产生类似于 BERT [bert-base-uncased](https://huggingface.co/bert-base-uncased) 架构的配置。
...
该模型还是 PyTorch [torch.nn.Module](https://pytorch.org/docs/stable/nn.html#torch.nn.Module)子类。将其用作常规 PyTorch 模块，并参考 PyTorch 文档以获取与一般用法和行为相关的所有事项。
...
为了作为解码器行为，模型需要使用配置中的`is_decoder`参数初始化为`True`。要在 Seq2Seq 模型中使用，模型需要使用`is_decoder`参数和`add_cross_attention`设置为`True`进行初始化；然后期望将`encoder_hidden_states`作为输入传递给前向传递。
...
一个 transformers.modeling_outputs.BaseModelOutputWithPoolingAndCrossAttentions 或一个 `torch.FloatTensor` 元组（如果传递 `return_dict=False` 或 `config.return_dict=False`）包含各种元素，取决于配置（QDQBertConfig）和输入。
...
QDQBertModel 的前向方法，覆盖了`__call__`特殊方法。
...
带有顶部`language modeling`头部的 QDQBERT 模型，用于 CLM 微调。
...
一个 transformers.modeling_outputs.CausalLMOutputWithCrossAttentions 或一个`torch.FloatTensor`元组（如果传递`return_dict=False`或`config.return_dict=False`）包含各种元素，取决于配置(QDQBertConfig)和输入。
...
QDQBertLMHeadModel 的前向方法，覆盖了`__call__`特殊方法。
...
一个 transformers.modeling_outputs.MaskedLMOutput 或一个`torch.FloatTensor`元组（如果传递了`return_dict=False`或当`config.return_dict=False`时），包括根据配置（QDQBertConfig）和输入的不同元素。
...
QDQBertForMaskedLM 的前向方法，覆盖了`__call__`特殊方法。
...
一个 transformers.modeling_outputs.NextSentencePredictorOutput 或一个`torch.FloatTensor`元组（如果传递`return_dict=False`或`config.return_dict=False`）包含各种元素，取决于配置（QDQBertConfig）和输入。
...
QDQBertForNextSentencePrediction 的前向方法，覆盖`__call__`特殊方法。
...
一个 transformers.modeling_outputs.MultipleChoiceModelOutput 或一个`torch.FloatTensor`元组（如果传递`return_dict=False`或`config.return_dict=False`时）包含根据配置（QDQBertConfig）和输入的各种元素。
...
QDQBertForMultipleChoice 的前向方法，覆盖了`__call__`特殊方法。
...
一个 transformers.modeling_outputs.TokenClassifierOutput 或一个`torch.FloatTensor`元组（如果传递`return_dict=False`或`config.return_dict=False`）包含各种元素，取决于配置（QDQBertConfig）和输入。
...
QDQBertForTokenClassification 的前向方法，覆盖了`__call__`特殊方法。
...
带有用于提取问答任务（如 SQuAD）的跨度分类头部的 QDQBERT 模型（在隐藏状态输出之上的线性层，用于计算`跨度起始 logits`和`跨度结束 logits`）。
