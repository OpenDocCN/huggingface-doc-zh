...
+   `guidance_scale` (`float`, *optional*, 默认值为7.5) — 较高的引导比例值鼓励模型生成与文本`prompt`密切相关的图像，但会降低图像质量。当`guidance_scale > 1`时启用引导比例。请注意，原始的潜在一致性模型论文使用了不同的CFG公式，其中引导比例减少了1（因此在论文公式中，当`guidance_scale > 0`时启用CFG）。
