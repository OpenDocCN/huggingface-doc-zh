...
然后，使用[TFAutoModelForImageClassification](/docs/transformers/v4.37.2/en/model_doc/auto#transformers.TFAutoModelForImageClassification)加载ViT以及标签映射：
...
加载图像处理器以预处理图像并将`input`返回为PyTorch张量：
