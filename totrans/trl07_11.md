...
+   *`*ref_model**` (`PreTrainedModelWrapper`, *可选*) — 用于KL惩罚的参考模型，Hugging Face变换器模型带有一个非正式语言建模头。查看`PreTrainedModelWrapper`的文档以获取更多细节。如果没有提供参考模型，训练器将创建一个具有与要优化的模型相同架构的参考模型，共享层。
